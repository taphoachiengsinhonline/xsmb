...
class TensorFlowService {
  ...
  decodeOutput(output) {
    ...
    for (let i = 0; i < 5; i++) {
        ...
        let candidates = validOutput
            .sort((a, b) => b.value - a.value)
            .slice(0, 5)
            .filter(item => item.value > 0.2)  // THÊM MỚI: Threshold 0.2
            .map(item => item.digit);
           
        // THÊM MỚI: Nếu không đủ, thêm random từ remaining
        if (candidates.length < 5) {
            const remaining = Array.from({length: 10}, (_, j) => j.toString()).filter(d => !candidates.includes(d));
            const shuffledRemaining = remaining.sort(() => Math.random() - 0.5);
            candidates = [...candidates, ...shuffledRemaining.slice(0, 5 - candidates.length)];
        }
           
        prediction[`pos${i + 1}`] = candidates;
    }
    ...
    for (const day of recentDays) {
        ...
        try {
            ...
            const inputSequence = sequenceDays.map(sequenceDay => {
                ...
                const EXPECTED_SIZE = 348;  // CẬP NHẬT MỚI: 346 + 2 từ triple
  ...
  async runLearning() {
    ...
    for (const pred of predictionsToLearn) {
        ...
        const inputTensor = tf.tensor3d([inputSequence], [1, SEQUENCE_LENGTH, 348]);  // CẬP NHẬT MỚI
  ...
  async runNextDayPrediction() {
    ...
    const expectedValues = SEQUENCE_LENGTH * 348;  // CẬP NHẬT MỚI
